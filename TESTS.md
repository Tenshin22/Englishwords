...
- Строки статусов выровнены, показывают прогресс `[текущий/всего]` и подсвечены по типу (PASS/XFAIL/XPASS/FAIL/ERROR).
- Шумные `print` из тестируемого кода скрыты; stdout теста выводится при FAIL/ERROR/XPASS или при `TESTS_VERBOSE=1`.
- В конце печатается сводка по статусам с временем прогона и список XFAIL с краткими комментариями.
- XFAIL снабжены тегами тематики (`UX`, `ROB`, `DATA`), сводка показывает их распределение.

Настройки:

- `TESTS_COLOR=0|1|auto` — выключить/включить цвет (по умолчанию `auto`, работает если stdout — терминал).
- `TESTS_VERBOSE=1` — показать stdout для успешных тестов.
- `TESTS_WIDTH=NN` — задать ширину переноса длинных сообщений (по умолчанию 96, минимум 60).
- `TESTS_ONLY=<substr>` — запускать только тесты, в имени которых есть `<substr>` (регистр не важен); в заголовке выводится выбранное количество.
- `TESTS_BRIEF=1` — выводить XFAIL в одну строку (обрезка с `…`).
- `TESTS_COMPACT=1` — компактный режим: все сообщения статусов в одну строку с обрезкой.
